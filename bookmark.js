// Zapiet Variant Restriction Audit (Bookmarklet Loader Script)
(function () {
  console.log("🔍 Loading Zapiet Audit Module...");
...
"45317878939925":["Regular Delivery","Ube Custard"],
"45317878972693":["Mini Delivery","Ube Custard"],
"45317879038229":["Regular Pickup","Strawberry Shortcake"],
"45317879070997":["Mini Delivery","Strawberry Shortcake"],
...
"45317874516245":["Family Size","Baked Mac"],
"45317874549013":["Party Size","Baked Mac"],
"45317873697045":["Family Size","Carbonara"],
"45317873729813":["Party Size","Carbonara"],
"45317872746773":["Family Size","Pancit Bam-I"],
"45317872779541":["Party Size","Pancit Bam-I"],
"45317872582933":["Family Size","Palabok"],
"45317872615701":["Party Size","Palabok"],
...
"49526281437461":["10 pax","Jr Spaghetti & Shanghai"],
"49526280126741":["10 pax","Pesto & Shanghai"],
"49526281830677":["10 pax","Mac & Shanghai"],
"49526281961749":["10 pax","Mac & Chicken"],
...
"45317870878997":["50 pax","Bundle 3"]
  ...
  function compress(list){
    ...
    const s=list.map(d=>new Date(d)).sort((a,b)=>a-b);
    const one=86400000;let out=[],st=s[0],p=s[0];
    const f=d=>d.toLocaleString("en",{month:"short",day:"2-digit"}).replace(" ","");
    for(let i=1;i<=s.length;i++){
      if(i<s.length&&s[i]-p===one){p=s[i];continue;}
      out.push(f(st)+(st.getTime()==p.getTime()?"":"-"+f(p)));
      st=s[i];p=s[i];
    } return out.join("; ");
  }

  const store=ZapietEats.getSelectedRestaurant();
  if(!store){alert("Select a store first.");return;}

  const pick={},deliv={};
  (store.menus||[]).forEach(m=>{
    (m.config?.store_pickup?.product_date_restrictions||[]).forEach(v=>pick[v.variant_id]=compress(v.restricted_dates));
    (m.config?.local_delivery?.product_date_restrictions||[]).forEach(v=>deliv[v.variant_id]=compress(v.restricted_dates));
  });

  const data=Object.keys(VARIANTS).map(id=>{
    const [v,p]=VARIANTS[id];
    const P=pick[id]||"none",D=deliv[id]||"none";
    return {id,variant:v,product:p,pick:P,del:D,miss:P=="none"&&D=="none"};
  });

  const missing=data.filter(r=>r.miss).length;

  const old=document.getElementById("zapiet-panel");if(old)old.remove();
  const box=document.createElement("div");
  box.id="zapiet-panel";
  box.style=`
  position:fixed;top:20px;left:20px;right:20px;bottom:20px;background:#fff;
  border:2px solid #111;z-index:999999;padding:10px;overflow:auto;font-family:Arial`;
  box.innerHTML=`
  <div style="display:flex;justify-content:space-between;">
    <div>
      <b style="font-size:18px">Zapiet Variant Audit</b><br>
      Store: ${store.name}<br>
      Missing: <span style="color:red;font-weight:bold">${missing}</span> / ${data.length}
    </div>
    <button onclick="document.getElementById('zapiet-panel').remove()">✕ Close</button>
  </div><br>
  <input id="zapiet-search" placeholder="Search…" style="padding:4px;width:230px">
  <button id="zapiet-csv" style="margin-left:8px;padding:4px">⬇ CSV</button>
  <table id="zapiet-tbl" style="width:100%;border-collapse:collapse;margin-top:8px">
    <tr style="background:#eef">
      <th>Product</th><th>Variant</th><th>ID</th><th>Pickup</th><th>Delivery</th><th>Status</th>
    </tr>
    ${data.map(r=>`
      <tr class="r" style="background:${r.miss?"#ffd4d4":"white"}">
        <td>${r.product}</td><td>${r.variant}</td><td>${r.id}</td>
        <td>${r.pick}</td><td>${r.del}</td>
        <td style="color:${r.miss?"red":"green"}">${r.miss?"Missing":"OK"}</td>
      </tr>`).join("")}
  </table>`;
  document.body.appendChild(box);

  document.getElementById("zapiet-search").oninput=e=>{
    const q=e.target.value.toLowerCase();
    document.querySelectorAll("#zapiet-tbl .r").forEach(r=>{
      r.style.display=r.innerText.toLowerCase().includes(q)?"":"none";
    });
  }

  document.getElementById("zapiet-csv").onclick=()=>{
    let csv="Product,Variant,ID,Pickup,Delivery,Status\n";
    document.querySelectorAll("#zapiet-tbl .r").forEach(r=>{
      if(r.style.display==="none")return;
      const t=[...r.children].map(td=>td.innerText.replace(/,/g," "));
      csv+=t.join(",")+"\n";
    });
    const a=document.createElement("a");
    a.href=URL.createObjectURL(new Blob([csv],{type:"text/csv"}));
    a.download=`zapiet-${store.id}.csv`;a.click();
  }
})();
